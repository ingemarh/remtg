 ...
 for fn=f'
  ...
  if strcmp(ext,'.png')
   if strcmp(prog,'ffmpeg')
    [dum1,dum2]=unix(sprintf('%s -r %d -pattern_type glob -i ''%s/*/%s.png'' -metadata title="%s %s %s %d" -v error -vcodec libvpx -qmax 50 -y -dn -an %s/%s.webm',prog,rate,dpath,name,dpath,t1,t2,np,odir,name));
   else
